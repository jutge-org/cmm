...
@Dereference = class Dereference extends Ast
    compile: (state) ->
        ...
        { type, result: new PointerMemoryReference(type, valueResult, new IntLit(0)), exprType: EXPR_TYPES.LVALUE, lvalueId: 'unknown', isConst, instructions: valueInstructions } #TODO: Fix this lvalueId: unknown, should construct the lvalueId with all successive operations applied to it as we go up
